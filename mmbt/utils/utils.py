...
def setup_testube_logger(cfg) -> pl_loggers.TestTubeLogger:
    try:
        job_id = os.environ["SLURM_JOB_ID"]
    except Exception:
        job_id = None
    base_log_path = cfg.savedir
    run_id=cfg.name+'_'+cfg.run_id
    p = Path(base_log_path)
    p.mkdir(parents=True, exist_ok=True)
    return pl_loggers.TestTubeLogger(
        save_dir=base_log_path,
        version=job_id + "_" + run_id if job_id else run_id,
        name="lightning_logs",
    )